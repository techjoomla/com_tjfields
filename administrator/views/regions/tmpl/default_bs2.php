...
<form
		action="<?php echo Route::_('index.php?option=com_tjfields&view=regions&client=' . $this->input->get('client', '', 'STRING')); ?>"
		...
		<?php if (JVERSION < '3.0'): ?>
			<div id="filter-bar" class="btn-toolbar">
				<div class="filter-search btn-group pull-left">
					...
					placeholder="<?php echo Text::_('COM_TJFIELDS_FILTER_SEARCH_DESC_REGIONS'); ?>"
					value="<?php echo $this->escape($this->state->get('filter.search')); ?>"
					class="hasTooltip"
					title="<?php echo Text::_('COM_TJFIELDS_FILTER_SEARCH_DESC_REGIONS'); ?>" />
				</div>
				...
				<div class="filter-select fltrt hidden-phone pull right">
					<?php if (JVERSION < '3.0') :
						echo HTMLHelper::_('select.genericlist', $this->countries, "filter_country", 'class="" size="1" onchange="document.adminForm.submit();" name="filter_country"', "value", "text", $this->state->get('filter.country'));

						echo HTMLHelper::_('select.genericlist', $this->sstatus, "filter_state", 'class="" size="1" onchange="document.adminForm.submit();" name="filter_state"', "value", "text", $this->state->get('filter.state'));
						?>
					<?php endif; ?>
				</div>
			</div>
							...
							<tr class="row<?php echo $i % 2; ?>">
								...
								<td>
									<?php if (isset($item->checked_out) && $item->checked_out) : ?>
										<?php echo HTMLHelper::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'regions.', $canCheckin); ?>
									<?php endif; ?>
									<?php if ($canEdit) : ?>
